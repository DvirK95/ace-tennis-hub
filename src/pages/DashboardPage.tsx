  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <StatCard icon={<Users className="h-5 w-5" />} label="People" value={people.length} accent="bg-info/10 text-info" />
        <StatCard icon={<UsersRound className="h-5 w-5" />} label="Groups" value={groups.length} accent="bg-success/10 text-success" />
        <StatCard icon={<CalendarCheck className="h-5 w-5" />} label="Events" value={events.length} accent="bg-primary/10 text-primary" />
