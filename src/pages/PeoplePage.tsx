...
export default function PeoplePage() {
  const {
    ...
    isFormOpen, setIsFormOpen, editingPerson,
  ...
  return (
    <div>
      <PageHeader
        ...
        action={<Button onClick={openCreate}><Plus className="h-4 w-4 mr-2" />Add Person</Button>}
      />
      <div className="mb-4">
      ...
      <DataTable
        ...
        actionColumn={(person: ClubUser) => (
          <div className="flex gap-1">
            <Button variant="ghost" size="sm" onClick={() => openEdit(person)}><Pencil className="h-3.5 w-3.5" /></Button>
            <Button variant="ghost" size="sm" onClick={() => handleDelete(person.id)}><Trash2 className="h-3.5 w-3.5 text-destructive" /></Button>
          </div>
        )}
      />
      <PersonFormDialog
