    ...
    <div className="flex items-center justify-between py-3 border-b last:border-0 px-4">
      <div className="flex items-center gap-6">
        <div>
          ...
          <p className="text-xs text-muted-foreground">{event.date} · {event.startTime}–{event.endTime}</p>
        </div>
      ...
      <div className="flex gap-2">
        ...
        <Button size="sm" variant="outline" onClick={() => onReject(event.id)} className="gap-1 text-destructive">
  ...
  return (
    <div>
      ...
      <div className="bg-card rounded-lg border shadow-card animate-fade-in">
        {pending.length === 0 ? (
          <p className="text-center py-12 text-muted-foreground text-sm">No pending events to review</p>
        ) : (
          pending.map((e) => (
            <ApprovalRow
              key={e.id}
              event={e}
              courtName={e.courtId ? (courtMap.get(e.courtId) ?? "—") : e.allCourts ? "All Courts" : "—"}
